...
## 0.7 ★ 2026-09-24 关键帧「不认定妆照」修复（用户报 + 现场取证）

### 0.7.1 症状与客观数字（`wv_faceid.py`，**同镜 / 同主体 / 各用自己的定妆照**）

| 版本 | 检出脸 | 宝玉 cos | 可卿 cos | 警幻 cos |
|---|---|---|---|---|
| Qwen 时代 同镜（09-23 11:09） | 2 张 | 0.49 | **0.536** | — |
| FLUX.2 重跑（09-24 08:29） | 3 张 | **0.271** | **0.170** | 0.457 |

判据：`>0.5` 同一人 / `0.35–0.5` 偏弱 / `<0.3` 基本不是同一人。⇒ 构图人数更像了，**参考图→脸的绑定垮了**。

### 0.7.2 根因（提示词侧，**自家引入**，2026-09-23 那版 FLUX.2 改写逻辑）

1. ★ **参考图指令被二次改写吃掉**：worker 先用官方口径写前缀 `Reference Image 1 2 3`，随后 `_flux2_slot_rewrite` 的
   `image\s*(\d+)` 又把刚写好的 `Image 1` 改成 `参考图 1` ⇒ 实际下发的正词是
   `（Reference 参考图 1 Reference 参考图 2 Reference 参考图 3）` —— **官方训练口径从未进过模型**。
2. **字面 N 残留**：同一段里还留着 Qwen 口径的 `不同 imageN 是不同的**人**`。
3. **身份类负词被整条丢**（`_flux2_fold_negative` 只拿 negative 打日志）。

### 0.7.3 修复（已随本次部署上线）

| # | 改哪 | 内容 |
|---|---|---|
| 1 | `worker/comfy_client.py` | 新增单一真源 `_image_edit_prompt()`：**槽位改写 → 加前缀 → 折负词**，顺序锁死（生产 / diag 共用，杜绝再漂移）；字面 N 按语境改写；负词身份组折算成**正向**约束句（正词已写则不重复补） |
| 2 | 同上 | 前缀槽位清单分隔符跟随语言（中文`、`英文`, `） |
| 3 | `api/.../JobService.java` | **定妆照固定 1:1**（`PORTRAIT_ASPECT`，不跟项目画幅）：1664 档 ⇒ **1344×1344** |
| 4 | `deploy/comfy/flux2_dev_edit_api.json` | 3 个参考图支链 `ImageScale`(到目标尺寸) → `ImageScaleToTotalPixels`(megapixels=1)：**保长宽比**（否则 1:1 定妆照会被 16:9 目标拉变形）+ 参考图预算回到官方 1MP（4096 token/张，原 6032） |

### 0.7.4 为什么定妆照要 1:1（用户 2026-09-24 裁定）

参考图能传递多少身份信息 ≈ **落在脸上的像素**，不是整图像素：

| | 旧（16:9） | 新（1:1） |
|---|---|---|
| 尺寸 | 1664×928 | 1344×1344 @1664 档（`ImageDims.of("1:1", 1664)`） |
| 实测脸宽 | 207–289px（占画面 ~4%） | 目标 ≥450–500px（~15–20%） |
| 送入 edit | 6032 token/张 | 1MP ⇒ 4096 token/张 |

⇒ **总像素降、脸占比升**；提分辨率不管用（脸的占比不变，token 却继续涨）。

> ⚠️ 尚未做（属另一变量，待 A/B）：`SubjectPrompts.portraitCore` 的取景约束强化（“人物占画面高度 ≥ 2/3”）。
> 验收口诀：同镜同 seed 重跑 → `wv_faceid.py` 出数（**cos >0.5 才算锚定成功**）；仍低就先查正词是不是又漂了。

---

### 0.7.5 身份绑定实验台账（2026-09-24 下午/晚上，**同 seed / 同 3 张定妆照 / 同 1664×928 / 同 20 步 guidance 4**）

> 判据用**同批排名**（谁的脸在自己身上最高、且明显高于其他人），不用 `cos>0.5` 硬门槛（用户 2026-09-24 裁定）。
> 工具：盒上 `/opt/weaveora/diag/wv_faceid.py`；对照脚本 `deploy/diag/wv_ab_bands*.py` / `wv_ab_refmp_vps.py`（在 VPS 上跑，`import /opt/weaveora/comfy_client.py`，与生产同一套注入逻辑）。

| 变量 | 臂 | 脸数 | 各脸最高 cos（宝玉/可卿/警幻） | 结论 |
|---|---|---|---|---|
| 基线（1MP、无位置） | A | 3 | 0.166 / 0.269 / 0.325 | 三人都不达标 |
| **①对称分带**（左/中/右） | B | 4 | 0.188 / 0.289、0.37 / 0.262 | **无效** |
| **③参考图预算 1MP→1.545MP**（保长宽比，4096→6032 token/张） | A(1MP) | — | 出图完成（482.8s，`weaveora_flux2_edit_00004_.png`） | **B 臂因盒子掉线中断，待重跑** |

**生产侧同时段实测**（英文口径正词 2770 字，已含「exactly 3 characters / exactly once」）：

| 任务 | 脸数 | 各脸最高 cos |
|---|---|---|
| 18:35（rev 90，中文口径） | 4 | 0.46、0.442 / 0.213 / 0.184 |
| 19:04（rev 93，**英文口径**，job …a0d2-1d1d） | 3 | 0.346 / 0.149 / 0.250 |

⇒ **提示词已经把话说尽**（列出 3 人、每人只出现一次、禁止互换/重复、槽位↔主体映射、负词折成正向句），模型仍会画 4 个人、且两位女性绑不住。
⇒ 归因指向 **FLUX.2 edit 在当前配置（3 参考图 / 1MP / 20 步 / guidance 4）下的多参考绑定强度不足**，不是提示词职责。

**下一步（待盒子恢复）**：重跑 ③ 的 1.545MP 臂 + faceid 对照；若仍无效 → 放开 Qwen-Image-Edit-2511 对照（同镜历史正例 0.49/0.536），或试“参考图数≤2 / 提高 steps”。

### 0.7.6 新功能（已上线，2026-09-24）：双击任务看「给模型的完整提示词」

任务列表**双击任一行**即可看到该次任务**真正下发给模型**的正词（逐字 + 字数）、负词（折前原文）、引擎参数（工作流/尺寸/steps/guidance 或 cfg/denoise/seed/LoRA）、参考图槽位↔主体对照，并可一键复制。
数据链：worker 上报 `finalPrompt/finalNegative/engineParams` → `/internal/jobs/{id}/complete` → `GenerationJob.applyPromptReport()` 追加进 `job.payload`（jsonb，不动 Flyway）→ 前端弹窗。
上线前的旧任务回退显示 API 侧 `positive_prompt` 并注明差异。
（顺手修：still 通路 media 是 5 元组 ⇒ `extra` 恒空 ⇒ P1 的“降级提示写进资产卡”**从未生效**，已补齐第 6 元素。）
