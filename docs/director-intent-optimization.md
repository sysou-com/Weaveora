...
## P4 参考图↔主体绑定（治“参考图没生效/用错图”）
**背景（女儿国 v8 实证，2026-09-10）**：用户上传唐僧参考图并改 S1 文案（英文正词其实已正确表达“背影→女王脸”），但 11:08 重跑任务的 payload 仍引用 **23:17 的旧参考图**；`briefs.constraints` 为空（选择从未落库），也没有“哪个主体对应哪张图”的关联，且重锚只换 prompt 不换 refs。

**已实施（✅ 上线）**
1. **参考选择随方案落库**：`plan.referenceAssets=[{assetId, subject}]`；参考图面板给选中图标注主体（如 `唐僧`），点保存修改或直接生成会自动保存。
2. **逐镜自动绑定**：带 subject 的参考图仅当该镜文案（action/zh/positive_prompt）提到该主体才绑定；未标注主体的始终绑定；全未命中则全带；并把 `The appearance of <subject> must strictly follow the provided reference image (identity, face and costume).` 追加进正词并重算 `prompt_md5`。
3. **重试/重跑重锚时重新解析参考图**（修掉沿用旧 job `referenceKeys` 的 bug）——本次 V8 事故的直接原因。
4. **导演 LLM 感知**：重导时注入“可用参考图主体清单”，要求把“形象以参考图为准”写进 positive_prompt、并在 action 保留主体名。
5. **任务列表「只看最近一轮」**：改为按 `镜号 + 类型 + 关键帧序号` 取最新一条（**不分状态**），重跑中/重跑失败的任务可见。

**仍需注意**：首轮新建时（还没有方案）选图仍走 brief 兜底=项目最新参考图；如需更严格可把主体标注也写进 brief。
