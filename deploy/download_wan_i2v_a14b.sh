...
#   落盘：/home/dataset-local/weaveora/models  （ComfyUI/models 是指向它的符号链接）
...
df -h /home/dataset-local | tail -1 | tee -a "$LOG"

# 前置空间检查：需要 ~35GiB，留 5GiB 余量
AVAIL_K=$(df -Pk /home/dataset-local | awk 'NR==2{print $4}')
