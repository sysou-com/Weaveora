...
## 9. 预设与试听标记（P7.2）
- **音色预设**：Web 里 `voice` 为下拉（中文女/中文男/英文女/英文男/日语女/韩语女/粤语女），可直接输入自定义值（如 GPU 机器上的参考音频路径做克隆）；`music_mood` 同为下拉+自定义（史诗磅礴/温暖治愈/紧张悬疑/空灵神秘/浪漫柔情/轻快活泼/古典中国风/电子科技感/悲伤低沉）。
- **试听**：「试听配音」（可逐镜）与「试听配乐」（按当前 music_mood，换情绪后点一次即重生成一条试听）。
- **试听产物隔离**：试听任务 payload 带 `preview=true`，落库 kind 为 `voice_preview` / `bgm_preview` —— 会出现在资产库（可播放）但**不会被正式渲染/导出选中**；正式产物才是 `voice` / `bgm`。
- 任务行标注 `配音·试听` / `配乐·试听` 便于区分。
