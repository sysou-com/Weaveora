...
补丁内容（版本 `2026-09-22.2`；`2026-09-14.1` 为轨迹锁人基线）：

| 文件 | 补了什么 |
|---|---|
| `latentsync/utils/face_detector.py` | **轨迹锁人**（点选/定妆照只播种，之后按「上一帧框 + 自累积人脸特征」逐帧跟住，绝不换人）；**质量闸门**（跟丢/脸太小<64px/侧脸>0.55 → 该帧不驱动）；`last_driven` 契约；`2026-09-22.2`：三个门槛改为**环境变量可调**（`WEAVEORA_LIPSYNC_MIN_FACE` / `_MAX_YAW` / `_HOLD_FRAMES` 抖动抑制，**默认值不变**） |
