 ...
 * 出图链路仍打旧地址 → 反参考图上传失败：
 * {@code urlopen error [Errno 111] Connection refused}。
 *
 * <p>同时 worker 机器上的 {@code /etc/weaveora/weaveora-gpu-worker.env} 是**回退值**，
 * 本端点管不到它（那是部署侧），页面会提示一并改。
 *
 ...
 */
public record GpuAddressSyncRequest(String host, Integer port, String oldHost) {
}
