...
日期：2026-09-04（**v2.1 瘦身：2026-09-19**，见 §0.1）  
文档用途：唯一产品 / 架构真源。实现模型**按 §0.1 分层阅读**后写入代码，不另发明架构。

> **版本沿革（历史快照，不代表当前设计）：** v1.2（首版）→ v1.3（7 项基础裁定：双轨前置/Redis Streams/去 Nacos/Java21+Boot3.4.5/本地存储/简化额度/单人里程碑）→ v1.4（中间件集中 VPS）→ v1.5（开发库本机）→ v1.6（双轨执行 #22）→ v1.7（一致性范围 #23）→ v1.8（定位收窄 #24/#25）→ v1.9（模型时效修正）→ **v2.0（当前）**；各版本细节与当时模型基准已归档至 `docs/archive/Weaveora-v2.0-已归档章节.md`，**勿当作现行设计引用**。
...
0. 给实现模型的硬性指令（约束总纲；含 0.1 高效使用本文档 / 0.2 大文件下载铁律 / 0.3 上下文与输出纪律）  
1. 命名与品牌  
2. 要解决的问题  
3. 目标用户  
4. 产品原则  
...
> **本节规范正文已收口到仓库根 `CLAUDE.md`「二、红线」**（该文件由 pi 每个会话自动注入；本节原文见 `docs/archive/Weaveora-v2.0-已归档章节.md`）。
> **优先级（冲突时）**：`CLAUDE.md` 红线（＝本节）> §30 已锁定决策 > 正文其他章节 > 现有代码。与代码冲突时**停下问产品**，不要擅自改产品语义。
> 红线文本**以 `CLAUDE.md` 为准**（原「以 Weaveora.md §0 为准」的表述由本指针取代）。

19 条索引（逐条正文见 `CLAUDE.md`）：①先读文档再写代码 ②GPU 推理不进 Java ③生成必过用户确认闸门并落库 ④共用 `Brief→PromptDraft→Confirm→Job→Asset`（视频加 Script/Scene/Shot/EditPackage）⑤Modulith 模块化单体 ⑥Vue 3 Web 为 MVP 唯一交付面 ⑦只有 PG + Redis + 对象存储 ⑧Job 可重试/取消/幂等（`idempotency_key`）⑨密钥不进仓库/前端 ⑩§30 已锁定，冲突问产品 ⑪中文文案 / 英文提示词 / 英文字段 ⑫演示前端≠生产实现 ⑬禁 `ddl-auto=update` ⑭禁 `ConcurrentHashMap` 存验证码与限流 ⑮不拷贝 MirrorTalk User 表 ⑯禁同步 HTTP 阻塞等 GPU ⑰ORM 锁 Spring Data JPA ⑱正文不锁模型版本（走 Model Preset / 配置）⑲大文件下载走 10 路分片 + 断点续传。
### 0.1 如何高效使用本文档（实现模型必读，v2.1）

> **v2.1（2026-09-19）修订**：原「每个实现会话完整读入全文」的约定**已废弃**——它与 `CLAUDE.md` 的分层阅读协议冲突，且不再反映实际体量（瘦身前为 2272 行 / 约 127 KB ≈ 3.2 万 tokens）。本次已把 §0.2/§0.3 等细则、§32/§33 等存档移出正文，**减少约 15 KB**。

1. **按任务分层读，不要每会话全文重读**：开场先读 **mtime 最新**的 `docs/交接快照-*.md`，再按 `docs/知识档案索引.md` §0「阅读优先级」路由到具体章节 / 文件。
2. **涉及产品语义 / 架构 / 状态机 / API 契约 / 数据模型变动** → 读本文档 **§0–§32 全文**。
3. **局部代码改动 / 修 bug** → 只读 §0 + §30 + 与本任务相关的章节。
4. **§0 / §30 是约束总纲**：与正文冲突时 §0 / §30 优先；与代码冲突时停下问产品（§0-10），不要擅改产品语义。
5. **模型 / 引擎一律读 Model Preset / 引擎配置（§7.8 / §23）**，不要从本文或历史章节推断版本。
6. **需要外部事实（模型 / 价格 / 竞争）时检索当下最新信息并标注来源 + 日期**，不要依赖本文或模型记忆（§0-18）。
7. **已归档区**（在 `docs/archive/Weaveora-v2.0-已归档章节.md`，不要回读）：原 §33 版本沿革 / §1.2 备选名 / §32 附录 / §0.2、§0.3 细则全文 / §28.1 开工顺序。
### 0.2 大文件下载铁律（2026-09-13 新增，必须逐条遵守）

> 全文（含事故背景与完整禁止表）已归档至 `docs/archive/Weaveora-v2.0-已归档章节.md`；运维侧精简版见 `docs/notes/运维铁律-GPU与生产.md` ⑦⑧。必须记住的三条：
>
> 1. **「大文件」= 单文件 ≥ 200 MiB**（模型权重 / 数据集 / 镜像 / 安装包 / 音视频）；`< 200 MiB` 才允许普通 `curl -L -o`。
> 2. **唯一允许的下载方式** = `deploy/windows/gpu_model_downloader.js`（10 路 Range 分片 + `.meta.json` 断点续传 + `.done` 标记 + 后台静默 + 每 10s 一行进度 + 404 立即失败）：
>    `node deploy/windows/gpu_model_downloader.js "<URL>" "D:\path\to\file.bin" 10`
> 3. **禁止**：`huggingface_hub.snapshot_download` / `hf_hub_download`、`modelscope.snapshot_download`、`git clone` / `git lfs clone` 大仓库、裸 `curl -o` / `wget` 单连接下载。
### 0.3 上下文与输出纪律（2026-09-16 新增，必须逐条遵守）

> 全文（含 14.5 MB 会话事故背景）已归档至 `docs/archive/Weaveora-v2.0-已归档章节.md`。摘要：
>
> 1. **按任务开新会话**：一个会话只推进一个阶段，阶段完成即输出状态快照并换会话；禁止跨多个不相关任务。
> 2. **禁止把原始 JSON / 日志倒进上下文**：远程命令只回聚合/摘要（`count(*)`、`left(col,120)`、`tail -n 3 | cut -c1-120`）；要看全量就落盘，上下文只留结论 + 文件路径。
> 3. **禁止写轮询循环**：长任务一律「后台脚本 + 日志文件」；agent 只做 1~2 次短查询；**单次等待 ≤ 60s**，禁止 `sleep 300/600`、禁止 `while sleep`。
> 4. **大文本走 context-mode 沙箱**（`ctx_execute_file` / `ctx_batch_execute`），禁止用普通 `cat` / 无过滤 `grep` 拉大文件。
> 5. **长输出先落盘再摘要**：> 2 KB 的产物先写文件，回复只给「路径 + 关键数字」。
## 1. 命名与品牌
...
> 已归档至 `docs/archive/Weaveora-v2.0-已归档章节.md`（CineLoom / DirectorLoom / LuminaForge / SceneSmith / Frameweaver 及其不选原因）。**命名与架构无关**，不再在正文占篇幅。
### 1.3 品牌视觉（实现必须遵守）
...
### 5.1 MVP（范围基线；v1.7 范围重排，v2.1 取消时间口径）

> 原文的「8–10 周」交付口径**已不适用**（实现自 2026-09 起按周推进并已超出该窗口）。**下方范围条目仍然有效**，进度看 mtime 最新的 `docs/交接快照-*.md`。
...
#### 7.5.1 定妆照与参考图（2026-09-16 裁定，实现必须遵守）

一致性链路的**唯一锚定物是「定妆照」**（`subjects[].portraitAssetId`）。据此定三条硬规则：
...
| 用途 | 唯一判据 | 说明 |
|---|---|---|
| **定妆照**的参考输入 | **勾选**（`checked`） | **与命名/主体标注无关**：勾了就算，没勾就不算。取消勾选＝留在列表但不用（缩略图变暗）；点图加入时默认就是勾选态 |
| **关键帧 / 出图**的参考 | **绑定的定妆照**（`subjects[].portraitAssetId`） | 参考图**不参与**出图锚定 —— 不存在“参考图把构图拉回去”这回事 |

> 另外：**定妆照生成成功后不自动绑定**（2026-09-16 用户裁定：生成结果大概率领不是想要的，必须由用户看过再手动选一版绑定：
> `主体 → 操作 ▾ → 把最新定妆照设为锚定图`）。

**位置框（区域%）的铁律与「脸过小」阈值（2026-09-16 裁定）**

1. **框 = 想让「生成出来的图」达到的构图效果**，**绝不**用它去裁/缩任何图片：
   送进模型的参考图/底图与库里原件 **md5 逐字节一致**（已实测）；区域坐标只有两个去向 ——
   ① 拼进正词（路线 A，生效中）；② `ConditioningSetAreaPercentage` 构图条件（`WEAVEORA_IMAGE_AREA_COND=1`，默认关）。
   **视频（motion）通路完全不读区域**；底图只做整帧等比缩放到视频分辨率（不得改成按主体框裁剪）。
2. 但框会**通过构图**间接决定脸的大小 —— 太小就掉身份。实测标定（本项目 6 张出图）：
   `框高 h=1.00 → 脸 47~59px（掉身份）`；`h=0.76 → 100px（相似度 0.609 ✓）`；`h=0.50 → 138px`；`h=0.64 → 166px`。
   拟合式：**脸宽 ≈ 0.09 × 画幅高 ÷ 框高**（1280×704 时 h=1.0→63px、0.6→106px、0.45→141px）。
3. 阈值与提示（位置总控已内置 `脸≈NNpx` 预告，**只提示不阻断**）：
   **< 40px** 检测极限以下（体检都测不出）｜**< 64px 红**（身份基本失效，出图易换脸）｜**64~96px 黄**（能用但脆弱）｜**≥ 96px 绿**（≥112px 稳）。
4. 伴随事实：**多主体同框时经常只检出 1 张脸**（实测 2~3 主体 → 检出 1 张），即使最大脸够大，其它角色的身份也可能没保住
   ⇒ 3 主体镜建议拆成 ≤2 主体（见 §7.6 与社区经验）。
...
1. 字段上限 8000 字（输入上限；用户原始要求「不要小于 4000」），AI 生成/更新**硬要求 ≥4000 字**；
   输入框为**分页文本域**（`PagedTextarea.vue`，不用内部滚动）：新建页 **15 行/页**、详情抽屉 **25 行/页**，
   溢出自动落到下一页，底部「上一页 / 下一页」+ **手机左右滑动换页**；每行字数按全角宽度估算（最坏情况，不会溢出）。
2. 两个 AI 动作：**AI 生成**（仅凭标题+类型）/ **AI 更新**（读全部要素 + 已写集数后重写）。
   两者都**先弹「设定字数」**（用户 2026-09-17：每个要素长度由用户设定，**上限 8000**；500 起、预设 1000/2000/4000/6000/8000、记住上次值），
   生成后再**先弹 diff**，用户确认后才写入字段（Q3）。
3. **AI 只产出值**：所有 AI 结果由用户确认后才会落库（铁律 ④ 的延伸）。
4. **输出侧：分段续写。**单次要「≥N 字」会被 `max_tokens=8000` 截断（`finish_reason=length` → 重试两次 → 422）。
   口径：每段约 2200 字（提示词硬上限 2600）、段数由目标字数算出（1–5 段）、拼接后达标；
   续写段带前文尾部 +「不得重复／不要总结」。
5. **输入侧：上下文预算。**六要素最大 4.8 万字，全量注入会让每段都重送一遍（成本/延迟线性膨胀）
   且会「中间丢失」。口径：**目标字段全文 + 其它要素「头 60%+尾 40%」摘要 + 每集标题摘要 + 仅最近 2 集正文**，
   总量封顶 12000 字（汉字≈token）；被压缩/省略的项记 INFO 日志（不静默）。见 `ScriptPrompts.context()`。
6. **结构先行：每段按提纲写。**先生成一份与段数 1:1 的提纲（每段 2–4 个子要点，≤600 字），
   再让每段「只写提纲第 K 条」——不跑偏、不重复、可显示进度；提纲同时回给前端展示（不入稿）。
   提纲生成失败**不阻断**正文（退化为按段数自行推进）。
7. **提纲会持久化并复用**（V18）：`scripts.outlines`（key=要素 → 每段一行），`script_episodes.outline`（本集的提纲）。
   「AI 更新」时若**段数与本次目标一致**则直接复用（省一次调用、结构不漂），段数变了就看新生成；
   弹窗里可勾「重新生成写作提纲」强制刷新；分集提纲随集保存，重开可见。
8. **分集字数同样可调**（弹窗内嵌，与要素同一套机制，两处各记一份上次值）；
   模型对长文长度控制不准（实测目标 1500 字写了 2692）→ 写超 **>1.5×** 时自动做一次
   **压缩重写**（保留人物/冲突/关键事件/**结尾钩子**，压回目标内），并在 `note` 里如实写明；
   压缩失败就保留原稿（宁可长，不丢内容）。本段提示里的「严格不超过」数字也与目标挂钩（≈目标×1.3，夹到 [800,2600]）。
9. **全剧记忆参与所有调用**：「精简的故事」（≤1500 字）作为最高优先级上下文注入字段生成 / 分集生成 /
   一致性检查 / AI 引导（空则显式写「尚无」）；它就是跨集的「整体结构/大纲」。
10. 另：网关 `JsonGuard.repairStrings()` 先修字符串内裸控制字符（模型写长中文段落时会写坏 JSON）；
   取正文用 `pickText()` 多键回退（`content/value/text/body/正文`）；某段失败/返回空正文时不丢已生成部分，
   并在结果 `note` 里如实告知「未达目标字数」。
   细节见 `docs/剧本模块-设计.md §12`。
...
| --- | --- | --- |
| Brief 分类、补全、中文剧本 | **由配置决定**（`weaveora.llm.*` = `WEAVEORA_LLM_BASE_URL` / `_API_KEY` / `_MODEL`，OpenAI-Compatible）；不写死任何型号 | JSON schema 强制；换模型只改环境变量 / profile（§0-18） |
...
- **本节只约束「调试 / 测试」的云通道**：测试时只走 `replicate.com` 官方接口（不自建网关、不接其它云商）；密钥只放 worker 环境变量，不进仓库/前端。
- **生产云通道已扩展（V12 / V13 / V14 / V15）**：除 Replicate 外还支持**网关通道**（例：火山方舟 `https://ark.cn-beijing.volces.com/api/v3/images/generations`）。网关没有机械可读的逐参数 schema，故由用户粘贴**示例请求**，后端解析出字段名与类型 → 生成与 Replicate 同形状的参数说明 + mapping（worker 复用同一套填参逻辑）。**本节测试模型只覆盖 Replicate 一侧，网关通道以用户填写的 schema / 示例为准。**
- **代码锚点**：`worker/cloud_client.py`（`WEAVEORA_REPLICATE_TEST_MODELS=1` 时才用本节测试模型）、`api/.../engine/GatewayModelProbe.java`、`ModelSchemaService.java`。
...
- 该口径仅用于** agent 调试/QA**，**不代表生产模型矩阵**：生产仍按 §11.2 的 Model Preset 中立路由（§30 #26）。
- **适用范围（重要）**：测试模型只在**显式开启调试时生效**（worker env `WEAVEORA_REPLICATE_TEST_MODELS=1`，或临时用 `WEAVEORA_REPLICATE_IMAGE_MODEL/VIDEO_MODEL` 覆盖）；**生产运行一律以用户引擎设置（Engine Settings）为准，不得静默替换为用户未配置的测试模型**；用户未配置模型时应报错提示去设置，而不是偷偷回退到测试模型。
...
- 迁移工具 **Flyway**，脚本 `api/src/main/resources/db/migration/V1__init.sql` … `V19__image_max_resolution.sql`（完整索引见 §14.2）。JPA 实体与脚本字段必须一致。
- **下方 SQL 只是 V1 基线**；V2–V19 的增量（新增表 / 列 / 触发器）见 §14.2。**字段真值 = 迁移脚本 + JPA 实体**（`ddl-auto=validate` 会在启动时强制对齐）。
...
### 14.1 剧本表（V17 `V17__script.sql` + V18 `V18__script_outline.sql`，我的剧本模块）
...
### 14.2 迁移索引（V1–V19，实现以迁移脚本为准）

> 本表用于对照「文档列的表」与「库里真实的表」；**新增结构一律只改迁移脚本**，不要只改本节。

| 迁移 | 内容 / 新增对象 |
| --- | --- |
| V1 `init` | 15 张核心表：`workspaces` `users` `memberships` `credit_wallets` `style_templates` `model_presets` `projects` `briefs` `prompt_revisions` `shot_drafts` `generation_jobs` `assets` `edit_packages` `credit_ledger` `audit_logs` |
| V2 `w3_worker` | `worker_nodes`（Worker 注册 / 心跳 / 能力上报） |
| V3 `marketplace` | 项目集市（分享 / 审批）+ 项目「最近更新」触发器 |
| V4 `market_marks` | `project_marks`（点赞 / 收藏，一人一票） |
| V5 `share_assets` | `project_share_assets`（集市只展示选中素材） |
| V6 `style_seeds` | 系统风格模板种子（`is_system`，`workspace_id NULL`） |
| V7 `engine_settings` | `user_engine_settings`（用户级引擎配置：图片 / 视频分别选 GPU 或云；凭据 AES 密文；任务路由列） |
| V8 `project_shot_duration` | 视频项目「每镜时长」偏好 |
| V9 `asset_shot_no` | `assets.shot_no` 冗余列 + 回填 |
| V10 `shot_locks` | `shot_locks`（分镜封版：按 `(project_id, shot_no)`，不按 `shot_id`） |
| V11 `engine_model_schema` | 云模型「调用参数说明」缓存 + 用户全局画质参数 |
| V12 `model_schema_error` | schema 拉取失败原因 + 网关通道参考图上限 |
| V13 `gateway_sample` | 网关通道「示例请求」文本 |
| V14 `model_presets` | 已配置模型库（同一用户多套模型配置） |
| V15 `engine_services` | 引擎服务地址（配音 / 配乐、对口型、转写、人脸） |
| V16 `gpu_max_resolution` | GPU 服务器「最大支持分辨率」（视频链路） |
| V17 `script` | `scripts` `script_episodes` `script_changes` `script_marks` |
| V18 `script_outline` | `scripts.outlines` + `script_episodes.outline`（提纲持久化与复用） |
| V19 `image_max_resolution` | 出图长边像素（全局生效于关键帧 / 定妆照 / 参考图；`null` = 默认 1280） |
...
```
weaveora-api（实际包结构，2026-09-19 核对）
  studio.weaveora
    identity/   用户、工作区、JWT、membership、登录验证码（Redis）
    project/    项目、Brief、风格模板（StyleTemplate*）
    director/   LLM 调用（LlmConfig / PlanReader）、revision、校验器、主体（Subject*）、AI 音频
    job/        状态机（`GenerationJob.state`）、建 / 撤销任务、出站 claim 认领、回执
    asset/      对象存储（StoragePort）、音频资产 / 配音预设
    export/     edit_list + 成片拼接（ConcatService）
    billing/    额度预扣 / 结算 / 释放（QuotaService，Redis 配额）
    engine/     生成引擎配置与模型库（UserEngineSettings / ModelPreset / 网关 schema 探测）
    script/     我的剧本（Script / Episode / Change / Mark、AI 精简 / 提纲 / 润色）
    shared/     错误码、全局异常处理
  infra/
    llm/        DirectorLlm / OpenAiDirectorLlm / JsonGuard / LlmProperties
    storage/    StoragePort + LocalFileStorageAdapter（**当前生产 `weaveora.storage.mode=local`；OSS 适配器尚未落地**）
    ws/         WebSocket 进度（JobWsHandler / WsConfig）
    obs/        Metrics / TraceFilter
    crypto/     AesGcm（引擎凭据密文）
```
...
> ⚠️ **待产品裁定的事实差异（未擅自改 §15.1 / §30）**：代码里 **未发现 Redis Streams**（全仓无 `XADD` / `XREADGROUP`）；`job/` 实际是「DB 状态 + worker 出站 `POST /internal/jobs/claim` 轮询认领」，Redis 当前只用于额度与登录验证码。
> 这与 §15.1「队列 = Redis Streams」、§0-16、§30 的口径不一致。按 §0-10「与代码冲突时停下问产品」，此处**只标记不改**——由产品决定是「改代码回 Streams」还是「改 §15.1/§30 承认 claim 轮询」。
> 同理：§15.1 写「生产 = 阿里云 OSS」，而 `application-prod.yml` 当前 `weaveora.storage.mode=local`，仓库内也**没有 OSS 适配器实现**（`infra/storage` 只有 `LocalFileStorageAdapter`）。

Python：

```
weaveora-worker（`worker/`，单层平坦布局，2026-09-19 核对）
  stub_worker.py     worker 主体：注册 → 心跳 → claim → 生成 → 上传 → complete
                     （`WEAVEORA_WORKER_MODE=stub|comfy`；纯 Python 无 app/ 包、无独立 hmac.py）
  comfy_client.py    ComfyUI 客户端（工作流注入 / 进度轮询 / 取件）
  cloud_client.py    云通道客户端（Replicate / 网关；§11.6 测试口径在此生效）
  cloud_image.py     云出图
  talk_client.py     对口型（talk 通道）
  audio_client.py    配音 / 配乐 / 转写
  range_download.py  大文件 10 路 Range 分片下载（§0.2）
  fetch_ref.py       参考图抓取
  e2e_local.py / qa_acceptance.py / test_*.py / diag_*.py   端到端与诊断脚本
```
...
```
weaveora/                      monorepo（实际结构，2026-09-19 核对）
  Weaveora.md                  产品 / 架构唯一真源
  CLAUDE.md                    会话入口与红线（pi 每会话自动注入）
  GPU服务器能力搭建指南.md      GPU 机器能力 / 搭建 / 运维指南
  docs/                        交接快照 / 专题设计 / notes / research / archive
  packages/schemas/            前后端共享契约
  api/                         pom.xml、src/main/java/studio/weaveora/、
                               src/main/resources/{db/migration,prompts}、scripts/
  web/                         Vue 3 创作台（src/{views,components,api,stores,router,theme}）
  worker/                      Python GPU Worker（见 §16.2）
  deploy/                      comfy/ windows/ audio/ face/ latentsync-node/ gpu-sync/
                               + 运维与诊断脚本（gpu2_*.sh / diag_*.py 等）
  Makefile
```

Maven：`weaveora-parent` BOM。包名 `studio.weaveora.<module>`。

> `app/`（二期 Flutter 移动审片，§18.2）**尚未创建**；`deploy/` 下没有 `compose/`、`k8s/`、`workflows/` 目录（原写法为规划值）。
...
> **本节 W1–W8 是「开工计划 + 验收清单」**（2026-08 制定，仍是验收口径）。
> **实际进度以 mtime 最新的 `docs/交接快照-*.md` 为准**——截至 2026-09-19 已远超 W5（Flyway 到 V19、我的剧本模块已上线、GPU 出图 / 对口型在跑）。
...
> 已归档至 `docs/archive/Weaveora-v2.0-已归档章节.md`（W1 文件级开工顺序 + 「第一周禁止事项」，2026-09 已执行完毕）。**当前进度看 mtime 最新的 `docs/交接快照-*.md`。**
## 29. 风险
...
| 27 | 云 API 测试口径（2026-09-10） | 云 API **测试**只走 **replicate.com**；出图固定 `stability-ai/stable-diffusion:ac732df83cea7fff18b8472768c88ad041fa750ff7682a21affe81863cbe77e4`；视频固定 `prunaai/p-video` 且 **draft=ON、分辨率 ≤720p**；**仅 agent 调试/QA 显式开启时临时使用（`WEAVEORA_REPLICATE_TEST_MODELS=1`）**，生产以用户引擎配置为准（§11.6） | 用户裁定（§11.6） |
...
> 正文已归档至 `docs/archive/Weaveora-v2.0-已归档章节.md`。两处现行位置：
>
> - **系统级默认负面词** = 代码 `api/src/main/java/studio/weaveora/director/plan/DirectorPlanValidator.java`
>   （`text, watermark, logo, subtitle, caption, signature, blurry, lowres, deformed, extra limbs, badly drawn, jpeg artifacts, ugly, nsfw`），可被模板覆盖。
> - **禁止事项重申** = `CLAUDE.md`「二、红线」，以那里为准。
## 33. 版本沿革（历史快照）

> 已归档至 `docs/archive/Weaveora-v2.0-已归档章节.md`（v1.3 → v2.0 变更快照）。其中的模型/引擎基准（Wan2.2 / SDXL 默认 / SVD 等）**已淘汰，禁止当作现行设计引用**；现行以 v2.0 正文与 §30 为准。
