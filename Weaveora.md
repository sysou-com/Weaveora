...
4. **长文必须分段续写**（2026-09-17 上线实测）：一次要「≥4000 字」会被 `weaveora` LLM 的
   `max_tokens=8000` 截断（`finish_reason=length` → 重试两次 → 422）。
   口径：每段约 2200 字（提示词硬上限 2600）、最多 3 段、拼接后 ≥4000；续写段带前文尾部 +「不得重复／不要总结」。
   另：网关 `JsonGuard.repairStrings()` 先修字符串内裸控制字符（模型写长中文段落时会写坏 JSON）；
   取正文用 `pickText()` 多键回退（`content/value/text/body/正文`），失败必须打日志带原文开头。
