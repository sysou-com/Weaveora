...
**产品与技术设计规格书 · v1.7**  
状态：**选型已锁定，v1.7 裁定已确认**（2026-09-04：v1.3–v1.6 各项 + 视频引擎/一致性范围）  
日期：2026-09-04  
...
> **v1.7 变更要点（2026-09-04，你确认 问题1=电影感级 / 问题2=B / 问题3=Wan）：** ⑪ **视频引擎与一致性范围（§30 #23）**：视频质量目标 = **电影感级**（非仅电商可用素材）；**motion 引擎 = 本地 Wan2.2**（5B 默认 / 14B 高质量档，能力分级；**SVD 废弃**）；一致性范围分档——MVP 先交付「产品/物体 + 虚构/IP 人物角色（非真实自然人）」一致性，**可识别真人 v1.0 解锁**（需肖像授权 + AI 标识合规）；一致性手段（IP-Adapter/参考图/种子锁）提前入 MVP，质量验收加「跨张/跨镜一致 + 电影感基线」。详见 §5 / §11 / §19 / §28 / §30 #23。
...
**核心价值线（v1.7：一致性优先 + 电影感视频）**

- 图片链路：口语 Brief → LLM 提示词 → 人工确认 → **一致性出图（参考图/IP-Adapter 锚定，产品/物体 + 虚构人物）** → 选图 / 再生成 / **跨张一致性校验**
- 视频链路：口语 Brief → LLM 剧本 + 分镜 + 逐镜提示词 → 人工确认 → 逐镜关键帧（一致性）→ **Wan2.2 本地 i2v（motion，电影感目标）** → 时间线 → 导出剪映草稿包
- 邮箱登录（手机号验证码可选），项目、草稿
...
### 11.3 视频两段式（强制，已锁定）+ 电影感质量目标（v1.7）
...
**视频质量目标 = 电影感级（v1.7，问题 1）：**

- 「电影感」由四层合成：① 高质量一致的关键帧（still pass 质量基线，含超分）；② Wan2.2 运动生成（5B 默认 / 14B 高配，能力上报分级）；③ 多镜编排 + 转场节奏（导演层 edit_plan）；④ 成片后处理（可选超分/调色，剪映完成精修）。
- **能力边界诚实标注**：本地消费卡（含 4090）受时长/分辨率/运动幅度物理上限约束；单镜目标 3–5s、720–1080p 档。真正「长镜头大片」超出本地引擎能力 → 归轨 1 高配云节点 / v1.5 云端视频 API 适配器（可灵/即梦/Veo 位保留）。
- 验收基线：电商/广告 15s 内成片（4–6 镜拼接）需达到「可商用、观感接近中高质视频」；电影感主观项用内部样片集对比定标。
...
## 28. 实施里程碑（v1.7 一致性优先 + 电影感视频，按周交付，单人）
...
| W7 | 质量验收基线（跨张/跨镜一致 + 电影感样片对比）、剪映适配器冒烟、安全扫描（主体分档） | 达标可商用闭环 |
| W8 | 管理后台最小集、额度、观测、打磨、轨 2 安装包（3090/4070/4080/4090） | 可给内测用户 |

GPU/轨 1 未到位时用 stub 保持接口；轨 1 GPU 到位切 `WEAVEORA_WORKER_MODE=comfy`（Q3-B）；轨 2 用户本地节点同款 worker（Q6）。MirrorTalk 的 Replicate 通道不受影响。
**不要从 Flutter 或管理后台开干。** 真人一致性与云端视频 API（可灵/即梦/Veo）属 v1.0，不在 W1-W8。
...
| 视频质量达不到「电影感」预期 | Wan2.2 分级 + 电影感由四层合成（关键帧质量/运动/多镜/后处理）；能力边界诚实标注；验收用样片对比定标 |
...
| 23 | 视频引擎与一致性 | **motion 引擎 = 本地 Wan2.2**（5B 默认 / 14B 高配，Apache 2.0 可商用；**SVD 废弃**）；视频质量目标 = **电影感级**（能力边界诚实标注，长镜头大片归轨 1 高配/v1.5 云端适配器）。**一致性 MVP 范围 = 产品/物体/场景 + 虚构/IP 人物（非真实自然人）**，IP-Adapter + 参考图 + 种子锁 + 2x 超分提前入 MVP；**可识别真人人像 v1.0 解锁**（肖像授权 + AI 标识合规）。质量验收含跨张/跨镜一致性 + 电影感基线 | v1.7 裁定（问题 1=电影感 / 2=B / 3=Wan） |
...
*结束。实现时以本文 v1.7 为唯一产品 / 架构真源。第 30 章已锁定（含 v1.3 #3 #4 #13–#19、v1.4 #20、v1.5 #21、v1.6 #22、v1.7 #23），可以开工。*
