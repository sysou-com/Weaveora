...
**产品与技术设计规格书 · v1.9**  
状态：**选型已锁定，v1.9 裁定已确认**（2026-09-04：v1.3–v1.8 各项 + 模型时效性修正与质量门禁）  
日期：2026-09-04  
...
> **v1.7 变更要点（2026-09-04，你确认 问题1=电影感级 / 问题2=B / 问题3=Wan）：** ⑪ **视频引擎与一致性范围（§30 #23）**：motion 引擎 = 本地 Wan2.2（5B 默认 / 14B 高质量档，能力分级；**SVD 废弃**）；一致性 MVP 范围 = 产品/物体 + 虚构/IP 人物（非真实自然人），IP-Adapter/参考图/种子锁提前入 MVP；可识别真人 v1.0 解锁。详见 §5 / §11 / §28 / §30 #23。

> **v1.8 变更要点（2026-09-04，你确认 1:A 2:A 3:A + deepseek-v4-pro 独立评审）：** ⑫ **定位与范围收窄（§30 #24）**：① 电影感 = **长期定位**，MVP 验收改可量化口径「15s 内成片、4-6 镜拼接、单镜 ≤6s、主体跨镜一致」；② **客群重切**：轨 2 主打 BYO 隐私/熟手提效、轨 1 主打电商垂直一致性工作流，不再正面撞即梦/可灵大众市场；③ **MVP 收窄**：轨 2 安装包 / admin 完整集 / 观测面板移出 MVP 主线。⑬ **视频事实固化（§11.3/§30 #25）**：本地与云端模型单次生成均有硬上限（Wan2.2-14B≈121 帧/7.5s；Veo3.1 单次 4/6/8s）；**长视频 = 分段生成 + 尾帧衔接 + 剪辑拼接**，此流程由产品封装（按目标时长自动拆镜）；本地 4090 定位 = 单镜素材生成器。

> **v1.9 变更要点（2026-09-04，你确认全收 5 条模型时效性修正）：** ⑭ **正文不锁模型/引擎具体版本（硬性指令 #18 新增）**：一切模型版本走 Model Preset / 配置项，正文只写能力档位。⑮ **时效修正**：LLM 默认 DeepSeek-V3 → **DeepSeek V4 系**（§10.1）；文生图质量基线 SDXL → **FLUX.1 系（Krea Dev / FP8 / GGUF 分级）为轨 1 默认，SDXL 降为兼容档**（§11.2）；视频引擎 Wan2.2 → **Wan 档位配置（本地主流 2.5/2.6，14B-FP8≈16G / FP16≈24G；2.7 走云 API）**，单次上限不再锁死 7.5s（§11.3/§30 #25）。⑯ **质量门禁**：跨张/跨镜一致性校验从人工验收升级为自动门禁（§11.4 增补）。详见 §0 #18 / §10.1 / §11.2 / §11.3 / §11.4 / §30 #23 #25。
...
- **无 GPU 用户**：Weaveora 代接入**预装好 SD 等服务的 GPU 服务器**（如 RTX 4090 ≈ ¥4/时），开箱即用；时租摊薄后单位产出成本低于按条计费云商。
- **已有 GPU 用户**：连自己的机器，省自建/调环境成本、省人力、提质。
- **主盈利 = 软件/工作流层**（导演层 + 一致性 + 资产库 + 剪映交付）；算力差价可作为盈利点但非主盈利。
...
**核心价值线（v1.8：一致性优先；电影感为长期定位，MVP 验收可量化）**

- 图片链路：口语 Brief → LLM 提示词 → 人工确认 → **一致性出图（参考图/IP-Adapter 锚定，产品/物体 + 虚构人物）** → 选图 / 再生成 / **跨张一致性校验**
- 视频链路：口语 Brief → LLM 剧本 + 分镜 + 逐镜提示词 → 人工确认 → 逐镜关键帧（一致性）→ **本地 Wan i2v（motion，版本走配置：2.5/2.6）** → 时间线 → 导出剪映草稿包。**长视频 = 分段 + 衔接 + 剪辑**：产品按目标时长自动拆镜（每镜 ≤ 当前引擎单次上限），尾帧衔接保持连贯，成片在剪映完成（v1.8/v1.9）
- 邮箱登录（手机号验证码可选），项目、草稿
...
- Model Preset：能力档位（SDXL 兼容 / FLUX 系 / Wan 5B·14B / 云视频 API 等），指向 Worker 里的 ComfyUI workflow；**具体模型版本为配置项，不写死在文档**（v1.9）。
- 管理员可热更新，不发版。
...
| ID（档位） | 用途 | MVP |
| --- | --- | --- |
| `sdxl_txt2img` | **兼容档出图**（低显存 / 生态兜底，LoRA/ControlNet 最全） | 必做 |
| `flux_txt2img` | **轨 1 默认质量档**（2026 主流：FLUX.1 系 / Krea Dev，FP8≈12G / GGUF 分级；**商用许可开工前书面核实**） | 必做（轨 1 默认） |
...
| `wan_i2v`（档位：5B / 14B） | 关键帧 → 运动；**版本走配置（本地主流 2.5 / 2.6；14B-FP16 ≈24G 需 4090，FP8≈16G）** | **必做（替代 SVD）** |
| `wan_txt2vid` | 文本短视频（本地可装档位） | 可选 |
| `cloud_video_api` | 高质量单镜（Wan 2.7 云 API / 可灵 3.0 / 即梦 Seedance / Veo 等，按当期最优配置） | v1.5 |
| `svd_img2vid` | ~~关键帧 → 短运动~~ | **废弃（v1.7；非商用许可 + 质量落后）** |
| `stub_txt2img` | 开发占位图 | 必做 |

模型文件 **不进 git**；授权（FLUX 系 / LTX / Hunyuan）开工前逐项书面核实。
...
- **可量化 MVP 验收**：目标时长 15s 内成片（4–6 镜拼接），单镜 ≤6s，主体跨镜一致，分辨率 720–1080p，达到「可商用、观感中高质」基线；电影感主观样片对比仅作内部定标，不作 MVP 硬门槛。
- **物理上限（v1.9 修订）**：所有视频模型**单次生成均有硬上限且随版本放宽**——不锁死固定秒数：本地 Wan 按当期档位（2.5/2.6 配置值；14B-FP8≈16G / FP16≈24G），云端 Veo3.1 单次 4/6/8s、可灵 3.0 / Wan 2.6 等可达 ~10–15s。**实现时从 Model Preset 读取当前单次上限**，据此拆镜。
- **长成片 = 分段 + 衔接 + 剪辑（产品封装逻辑）**：按目标时长 + 当前引擎单次上限自动拆镜 → 逐镜生成 → 尾帧衔接法保连贯（上段尾帧作下段参考首帧）→ 多镜时间线在剪映完成拼接/转场/字幕/音乐/调色 → 15–60s 成片。
- **本地 4090 定位 = 单镜素材生成器**（3–7s/镜）；要更高单镜质量（电影感镜）走轨 1 高配云节点或 v1.5 云端视频 API。
...
- **质量门禁（v1.9）**：一致性产出在**入库前自动校验**——同一参考主体的相似度比对（产品/虚构人物用特征向量阈值，图片与视频关键帧一致校验），低于阈值自动重 roll 或标记 `low_consistency` 不入主库；另加基础质量过滤（黑图/纯色/分辨率过低/损坏文件）。门禁通过才计入额度结算，减少用户「大海捞针」式挑选。
...
## 28. 实施里程碑（v1.8：一致性优先 + 可量化验收，按周交付，单人）
...
| W6 | 电商套图批量、时间线粗预览、edit_list 导出、README | 套图一致可交付；能送剪映 |
| W7 | 质量验收基线（15s 内成片、4-6 镜、跨镜一致、单镜 ≤6s 可商用）、剪映适配器冒烟、安全扫描（主体分档） | 达标可商用闭环 |
| W8 | 打磨（额度 simplified、观测最小集、安全扫描）；轨 2 安装包移出主线为可选后续 | 可给内测用户 |
...
| 视频成片达不到「电影感」 | MVP 验收用可量化口径（15s/4-6 镜/单镜 ≤6s/跨镜一致）；电影感为长期定位，靠轨 1 高配/v1.5 云端 API 渐进（v1.8） |
...
2026-09-02 你确认：第 1–3、6–11 条同意；第 4、5 条按 MirrorTalk 实测裁定。2026-09-04 v1.3 裁定修订 #3 #4 并新增 #13–#19；同日 v1.4 新增 #20（中间件集中部署）、v1.5 新增 #21（开发库本机过渡）。实现模型按本表开工，不得再问。
...
| 22 | 双轨执行 | **轨 1 云 GPU + 轨 2 本地 GPU（BYO）并存**：轨 1 = 用户用系统 + 租我/指定供应商的 GPU（Q4-A 租整机自跑 worker，自建 ComfyUI），MVP 轨 1 先行（Q3-B）；轨 2 = 用户自带本地 GPU 连自己工作区，MVP 支持 Windows + RTX 3090/4070/4080/4090，安装困难时陪装（Q6）。**Worker 协议双轨同构：一律出站 HTTPS 连 API 拉 Job + OSS 直传，不反向直连 Redis**（Q2-A）。轨 2 客群 = 已自建 ComfyUI 的创作者/工作室。**双轨定位备注（2026-09-04 澄清）：目的 = 降低用户使用门槛的两种入口（轨 1 代接入预装好 SD 等服务的 GPU 服务器，无 GPU 用户开箱即用，RTX 4090 ≈ ¥4/时，单位产出成本低于按条计费云商）；主盈利 = 软件/工作流层，算力差价非主盈利（可作盈利点但不主打）** | v1.6 裁定（1-6 确认）+ 定位澄清 |
| 23 | 视频引擎与一致性 | **motion 引擎 = 本地 Wan（版本走配置：2.5/2.6，5B / 14B 档，Apache 2.0 可商用；SVD 废弃）**；文生图轨 1 默认 = **FLUX.1 系（Krea Dev / FP8 / GGUF 分级），SDXL 降兼容档**（v1.9）；**一致性 MVP 范围 = 产品/物体/场景 + 虚构/IP 人物（非真实自然人）**，IP-Adapter + 参考图 + 种子锁 + 2x 超分提前入 MVP；**可识别真人人像 v1.0 解锁**（肖像授权 + AI 标识合规）；**质量门禁 = 一致性自动校验 + 基础质量过滤，入库前执行（v1.9）** | v1.7 裁定 + v1.9 时效/门禁修订 |
| 24 | 定位与范围（v1.8） | **电影感 = 长期定位，MVP 验收可量化**：15s 内成片、4-6 镜拼接、单镜 ≤6s、主体跨镜一致、720-1080p「可商用中高质」。**客群 = 两种降门槛入口（§22 备注）**：轨 1 面向无 GPU、要开箱即用的用户（含电商/设计师，代接入预装 GPU）；轨 2 面向已有 GPU、要省自建/人力/提质的用户。**MVP 收窄**：轨 2 安装包 / admin 完整集 / 观测面板移出 MVP 主线 | v1.8 裁定（1:A 2:A 3:A + deepseek-v4-pro 评审）+ §22 澄清 |
| 25 | 视频物理上限与长成片路径 | 单次生成硬上限随版本放宽、**从 Model Preset 读当前值，不锁死秒数**（本地 Wan 按当期配置；云端 Veo3.1 4/6/8s、可灵 3.0 / Wan 2.6 可达 ~10–15s）；**长成片 = 分段生成 + 尾帧衔接 + 剪辑拼接**，由产品封装（按目标时长 + 当前引擎上限自动拆镜）；本地 4090 定位 = 单镜素材生成器 | v1.8 裁定 + v1.9 修订 |
...
2. 建「视频」项目 12 秒 16:9 → 输入「一只纸船穿越雨夜城市」→ 得到 ≥4 镜剧本，时长和为 12s ±0.5 → 改第 2 镜提示词 → 只确认第 2 镜仍不能生成全片 → 确认全片 → 先出 4 张关键帧（同一主体一致）→ motion 出短运动视频（Wan）→ 导出 zip 内有 `edit_list.json` 与 README。
...
*结束。实现时以本文 v1.9 为唯一产品 / 架构真源。第 30 章已锁定（含 v1.3 #3 #4 #13–#19、v1.4 #20、v1.5 #21、v1.6 #22、v1.7 #23、v1.8 #24 #25、v1.9 时效/门禁修订），可以开工。*
