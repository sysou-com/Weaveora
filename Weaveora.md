...
**产品与技术设计规格书 · v1.8**  
状态：**选型已锁定，v1.8 裁定已确认**（2026-09-04：v1.3–v1.7 各项 + deepseek-v4-pro 独立评审后的定位/范围收窄）  
日期：2026-09-04  
...
- 视频链路：口语 Brief → LLM 剧本 + 分镜 + 逐镜提示词 → 人工确认 → 逐镜关键帧（一致性）→ **Wan2.2 本地 i2v（motion）** → 时间线 → 导出剪映草稿包。**长视频 = 分段 + 衔接 + 剪辑**：产品按目标时长自动拆镜（每镜 ≤6s），尾帧衔接保持连贯，成片在剪映完成（v1.8）
- 邮箱登录（手机号验证码可选），项目、草稿
...
- 镜头级视频增强（Wan2.2-14B 档成熟化 / 云端视频 API 适配器：可灵、即梦、Veo）
- 旁白 TTS、字幕轨、BGM 推荐
- 团队空间、评论、审片
- 剪映草稿更完整（转场、关键帧、字幕样式）
- 管理后台：模型、队列、成本看板
...
| `wan22_i2v_5b` | 关键帧 → 运动（默认档，电影感可用） | **必做（v1.7 替代 SVD）** |
| `wan22_i2v_14b` | 高质量运动（24G 卡 / 轨 1 高配档） | 按节点能力 |
| `svd_img2vid` | ~~关键帧 → 短运动~~ | **废弃（v1.7；SVD 非商用许可 + 质量落后）** |
| `animatediff_txt2vid` | 文本短视频 | 可选 |
| `hunyuan_txt2vid` | 高质量短视频 | 可选（授权需核） |
...
- **物理上限（v1.8 固化，#25）**：本地（Wan2.2-14B ≈ 121 帧 / 7.5s）与云端（Veo3.1 单次 4/6/8s）模型**单次生成均有硬上限**；不存在单次生成电影级长视频的模型。
- **长成片 = 分段 + 衔接 + 剪辑（产品封装逻辑）**：按目标时长自动拆镜（每镜 ≤6s）→ 逐镜生成 → 尾帧衔接法保连贯（上段尾帧作下段参考首帧）→ 多镜时间线在剪映完成拼接/转场/字幕/音乐/调色 → 15–60s 成片。
- **本地 4090 定位 = 单镜素材生成器**（3–7s/镜）；要更高单镜质量（电影感镜）走轨 1 高配云节点或 v1.5 云端视频 API。
- 用户 UX：选目标时长（15/30/60s）时产品自动拆镜并告知单镜上限，不让用户面对「只能生成 7.5s」的挫败。
...
- 能力上报：GPU 型号 / 显存 / 已装 workflow 与模型 / ComfyUI 版本；导演层与下单校验能力匹配。**视频引擎分级（v1.7）**：节点上报可用档位（Wan2.2-5B / 14B / 仅 SDXL 出图），导演层按档位给 motion 参数；显存不足自动降档或提示。
...
| W5 | 轨 1 云 GPU 接通 + **Wan2.2 i2v（motion）关键帧→mp4（5B 档）** | 确认关键帧后能出短运动视频 |
...
| 本地视频引擎授权雷 | 默认 Wan2.2（Apache 2.0）；SVD 废弃；LTX/Hunyuan 仅作 v1.5 观察且需核授权（v1.7） |
...
| 23 | 视频引擎与一致性 | **motion 引擎 = 本地 Wan2.2**（5B 默认 / 14B 高配，Apache 2.0 可商用；**SVD 废弃**）；**一致性 MVP 范围 = 产品/物体/场景 + 虚构/IP 人物（非真实自然人）**，IP-Adapter + 参考图 + 种子锁 + 2x 超分提前入 MVP；**可识别真人人像 v1.0 解锁**（肖像授权 + AI 标识合规） | v1.7 裁定（问题 2=B / 3=Wan） |
...
| 25 | 视频物理上限与长成片路径 | 单次生成硬上限（本地 Wan2.2-14B≈7.5s；Veo3.1 单次 4/6/8s）；**长成片 = 分段生成 + 尾帧衔接 + 剪辑拼接**，由产品封装（按目标时长自动拆镜，每镜 ≤6s）；本地 4090 定位 = 单镜素材生成器 | v1.8 裁定（事实核查） |
...
2. 建「视频」项目 12 秒 16:9 → 输入「一只纸船穿越雨夜城市」→ 得到 ≥4 镜剧本，时长和为 12s ±0.5 → 改第 2 镜提示词 → 只确认第 2 镜仍不能生成全片 → 确认全片 → 先出 4 张关键帧（同一主体一致）→ motion 出短运动视频（Wan2.2）→ 导出 zip 内有 `edit_list.json` 与 README。
...
*结束。实现时以本文 v1.8 为唯一产品 / 架构真源。第 30 章已锁定（含 v1.3 #3 #4 #13–#19、v1.4 #20、v1.5 #21、v1.6 #22、v1.7 #23、v1.8 #24 #25），可以开工。*
